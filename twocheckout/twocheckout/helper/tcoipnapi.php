...
class TcoIpnApi
{
    ...
    const PAYMENT_METHOD = 'twocheckout';
     ...
     */
    public function __construct($_ipn_params, $_order_number, $_current_method)
    {
        if (!class_exists('TcoData')) {
            require VMPATH_PLUGINS . '/vmpayment/twocheckout/twocheckout/helper/tcodata.php';
        }
        $this->_tco_helper = new TcoData();
        $this->_ipn_params = $_ipn_params;
        $this->_order_number = $_order_number;
        $this->_secret_key = $_current_method->twocheckout_secret_key;
        ...
        if (isset($params['FRAUD_STATUS'])) {
            ...
            switch (trim($params['FRAUD_STATUS'])) {
                case self::FRAUD_STATUS_DENIED:
                    ...
                    $order['comments'] = vmText::sprintf('VMPAYMENT_TWOCHECKOUT_PAYMENT_FRAUD_STATUS_DENIED', $this->_order_number);
                    break;
                ...
                case self::FRAUD_STATUS_APPROVED:
                    ...
                    $order['comments'] = vmText::sprintf('VMPAYMENT_TWOCHECKOUT_PAYMENT_FRAUD_STATUS_APPROVED', $this->_order_number);
                    break;
            }
